...
counter_plays = []
spills = []
spillsDict = {'teams': [], "net spill yards": [], "total spill snaps": [], "average spill yards": []}
boxes = []
boxesDict = {'teams': [], "net box yards": [], "total box snaps": [], "average box yards": []}
...
for play in counter_plays:
    ...
    if play.fit == 'spill':
        for player in play.players:
            if player.ps_EDGE == True:
                was_found = False
                for new_team in spills:
                    if new_team.name == player.team:
                        was_found = True
                        new_team.total_spill_snaps += 1
                        new_team.net_spill_yards += play.yards_gained
                if was_found == False:
                    team = Team(player.team) 
                    team.net_spill_yards = play.yards_gained
                    team.total_spill_snaps += 1
                    spills.append(team)
        ...
        for player in play.players:
            if player.ps_EDGE == True:
                was_found = False
                for new_team in boxes:
                    if new_team.name == player.team:
                        was_found = True
                        new_team.total_box_snaps += 1
                        new_team.net_box_yards += play.yards_gained
                if was_found == False:
                    team = Team(player.team) 
                    team.net_box_yards = play.yards_gained
                    team.total_box_snaps += 1
                    boxes.append(team)
print('spills')
for team in spills:
    average_yards = team.get_average_spill_yards()
    spillsDict['teams'].append(team.name)
    spillsDict['net spill yards'].append(team.net_spill_yards)
    spillsDict['total spill snaps'].append(team.total_spill_snaps)
    spillsDict['average spill yards'].append(average_yards)
print('boxes')
for team in boxes:
    average_yards = team.get_average_box_yards()
    boxesDict['teams'].append(team.name)
    boxesDict['net box yards'].append(team.net_box_yards)
    boxesDict['total box snaps'].append(team.total_box_snaps)
    boxesDict['average box yards'].append(average_yards)

spillsDF = pd.DataFrame(data = spillsDict)
boxesDF = pd.DataFrame(data = boxesDict)
spillsDF.to_csv('teamSpills.csv')
boxesDF.to_csv('teamBoxes.csv')
